...
import EditOrderShip from './EditOrderShip';
import EditStatusShiper from './EditStatusShiper';
...
function HistoryComponent() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await shiperApi.historyshiper();
    ...
    const mapOrderList = data.map((item) => {
      ...
      const order = {
        ...
        item.userOrder[0].address1.street_name +
         ...
         date_order: item.userOrder[0].createdAt,
         date_ship: item.userOrder[0].updatedAt,
  ...
  const columns = [
    ...
    },
    {
        ...
        key: 'date_ship',
        sorter: (a, b) => {
          return a.date_ship - b.date_ship;
        },
        render: (date) => {
          const _date = [
            moment(date).format('DD-MM-YYYY hh:mm:ss')
